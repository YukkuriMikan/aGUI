...
namespace ANest.UI {
	...
	public class aTextMeshProUguiEditor : UnityEditor.Editor {
		#region Fields
		private SerializedProperty m_stringTableProp;    // StringTable参照プロパティ
		...
		private SerializedProperty m_rawTextProp;          // ルビタグ付き元テキストプロパティ
		...
		private void OnEnable() {
			...
			m_rawTextProp = serializedObject.FindProperty("m_rawText");
		...
		public override void OnInspectorGUI() {
			...
			// TMP標準インスペクタのテキスト変更を検知してルビ解析を実行
			if(m_tmpEditor != null) {
 			...
 			if(EditorGUI.EndChangeCheck()) {
					foreach(var t in targets) {
						var tmp = (aTextMeshProUgui)t;
						// TMPエディタがm_textを直接書き換えるので、textプロパティ経由でルビ解析に通す
						var editedText = tmp.GetParsedText() ?? "";
						// m_textから直接読み取り（TMPエディタが書き換えた値）
						serializedObject.Update();
						var tmProp = serializedObject.FindProperty("m_text");
						if(tmProp != null) editedText = tmProp.stringValue;
						tmp.text = editedText;
						m_rawTextProp.stringValue = tmp.RawText;
